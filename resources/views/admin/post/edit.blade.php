...
{{--                        {{ dd($tags->pluck('id')->toArray(), $post->tags->pluck('id'))->toArray() }}--}}
{{--                        {{dd($tags->pluck('id')->toArray())}}--}}
                    </div><!-- /.col -->
                    <div class="col-sm-6">
                    ...
                    <div class="col-12">
                        <form action="{{route('post.update', $post->id)}}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control col-4 @error('title') is-invalid @enderror"
                                       name="title" placeholder="Введите название поста" value="{{$post->title}}">
                                @error('title')
                                <div class="text-danger text-nowrap">{{$message}}</div>
                                @enderror
                            </div>
                            <div class="form-group @error('content') is-invalid @enderror">
                                <textarea id="summernote" name="content" cols="60"
                            ...
                            <div class="form-group">
                                <label for="exampleInputFile">Обновить картинку для превью</label>
                                <div class="w-25 mb-2">
                                    <img @if($post->preview_image == null)
                                         alt="Изображения нет для данного поста"
                                         @else
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control col-4" name="category_id">
                                    ...
                                    @foreach($categories as $key => $category)
                                        <div>hello</div>
                                        <option value="{{$key}}"
                                                @if($post->category_id == $key) selected
                            ...
                            <div class="form-group col-4">
                                ...
                                <select name="tag_ids[]" class="select2" multiple="multiple"
                                    ...
                                    @foreach($tags as $tag)
                                        ...
                                        <option {{(in_array($tag->id, $post->tags->pluck('id')->toArray())) ?  'selected' : "Выберите теги"}} value="{{$tag->id}}" > {{$tag->title}} </option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                <button type="submit" class="btn btn-primary col-2" value="Добавить">Обновить</button>
                                <a href="{{route('post.index')}} ">
                                    <button type="button" class="btn btn-primary" style="margin-left: 16rem;">Назад
                                    </button>
                                </a>
                            </div>
                        </form>
